...
import { Linking,ScrollView,Image} from "react-native";
import { PricingCard } from 'react-native-elements';

export default class sewing extends React.Component {
  state = {
    persons: [],
    login: false,
  }
  ...
  componentDidMount() {
    api.get(`/empresas`)
      .then(res => {
        const persons = res.data.rows.filter(person => (person.tipoemp ==10));
        const login = true
